...
# def generate_ref_num(prefix="BBA") -> str:
#     timestamp = datetime.now().strftime("%d%m%y")
#     suffix = ''.join(random.choices(string.ascii_uppercase + string.digits, k=3))
#     return f"{prefix}{timestamp}{suffix}"
...
# def _safe_email(e):
#     e = _safe_str(e).lower()
#     return e if e and re.match(r"^[^@\s]+@[^@\s]+\.[^@\s]+$", e) else None
...
# def _dob_to_iso(dob_str: str):
#     """
#     Accepts '09-08-2000' or '09/08/2000' -> '2000-08-09'.
#     If already '2000-08-09', returns as-is (without timezone).
#     """
#     if not dob_str:
#         return None
...
# def _build_street(a1, a2, a3):
#     parts = [_safe_str(a1), _safe_str(a2), _safe_str(a3)]
#     parts = [p for p in parts if p]
#     return ", ".join(parts) if parts else None

# def _is_valid_pin(s) -> bool:
#     return len(_digits_only(s)) == 6

# def _region_code_for_state(state_name: str) -> str:
#     code = STATE_CODE_MAPPING.get(_safe_str(state_name), 97)
#     return f"{code:02}"

# def _pick_best_address(prefill_addresses: list, pan_addr: dict | None):
#     """
#     Choose one address for CIBIL:
#       1) From prefill: valid 6-digit pin and has state; if multiple, prefer with city.
#       2) Fallback to PAN address if valid.
#       3) If none valid, return None.
#     """
#     best = None
#     # 1) Prefill list
#     if isinstance(prefill_addresses, list):
#         valid = [a for a in prefill_addresses if _is_valid_pin(a.get("postal_code")) and _safe_str(a.get("state"))]
#         if valid:
#             with_city = [a for a in valid if _safe_str(a.get("city")) and not _digits_only(a.get("city"))]
#             chosen = with_city[0] if with_city else valid[0]
#             best = {
#                 "street": _build_street(chosen.get("first_line_of_address"), chosen.get("second_line_of_address"), chosen.get("third_line_of_address")),
#                 "city": _safe_str(chosen.get("city")) or None,
#                 "state": _safe_str(chosen.get("state")),
#                 "pin": _digits_only(chosen.get("postal_code")),
#             }

#     # 2) PAN fallback
#     if not best and isinstance(pan_addr, dict):
#         pin = pan_addr.get("pin_code")
#         state = _safe_str(pan_addr.get("state"))
#         if _is_valid_pin(pin) and state:
#             best = {
#                 "street": _build_street(pan_addr.get("address_line_1"), pan_addr.get("address_line_2"), pan_addr.get("address_line_3") or pan_addr.get("address_line_5")),
#                 "city": _safe_str(pan_addr.get("address_line_5")) or None,
#                 "state": state,
...
#             # 1) Mobile-to-Prefill (PAN + demographics)
#             print("📌 Fetching PAN using Mobile to Prefill API with name_lookup: 1.")
#             client_ref_num = generate_ref_num()
#             mobile_to_prefill_payload = {
#                 "client_ref_num": client_ref_num,
...
#             }
#             mobile_to_prefill_resp = await client.post(
#                 MOBILE_TO_PREFILL_URL, headers=HEADERS, json=mobile_to_prefill_payload
#             )
#             print(f"🔍 Mobile to Prefill API Response Status [{mobile_to_prefill_resp.status_code}]")
#             try:
#                 mobile_to_prefill_data = mobile_to_prefill_resp.json()
...
#                 print(f"❌ Failed to parse mobile_to_prefill_resp JSON: {e}")
#                 print(f"❌ Raw Response Content: {mobile_to_prefill_resp.content}")
#                 raise HTTPException(status_code=500, detail="Invalid Mobile to Prefill JSON")

#             print("📋 Full Mobile to Prefill API Response:", mobile_to_prefill_data)
#             message = _safe_str(mobile_to_prefill_data.get("message")).lower()
#             if message == "no record found":
#                 raise HTTPException(status_code=404, detail="No record found for the given mobile number.")

#             prefill_result = mobile_to_prefill_data.get("result") or {}
#             final_pan_number = _safe_str(prefill_result.get("pan"))
#             if not final_pan_number:
#                 raise HTTPException(status_code=400, detail=f"PAN not returned in Mobile to Prefill: {mobile_to_prefill_data}")

#             print(f"✅ Extracted PAN number: {final_pan_number}")

#             # 2) PAN Supreme verification
#             pan_supreme_resp = await client.post(PAN_SUPREME_URL, headers=HEADERS, json={"pan": final_pan_number})
#             pan_supreme_data = pan_supreme_resp.json()
#             print(f"🔍 PAN Supreme API Response: {pan_supreme_data}")

#             if str(pan_supreme_data.get("status")) != "1":
#                 raise HTTPException(status_code=400, detail=f"PAN Supreme verification failed: {pan_supreme_data.get('message', 'No message')}")

#             pan_details = pan_supreme_data.get("result") or {}
#             print(f"✅ PAN Supreme Details: {pan_details}")

#             # 3) Build normalized identity fields
#             # Name
#             name_prefill = _safe_str(prefill_result.get("name"))
#             first_name = _safe_str(pan_details.get("first_name")) or (name_prefill.split(" ")[0] if name_prefill else "")
#             last_name  = _safe_str(pan_details.get("last_name"))  or (name_prefill.split(" ")[-1] if name_prefill else "")

#             # DOB
#             dob_iso = _dob_to_iso(prefill_result.get("dob")) or _safe_str(pan_details.get("dob"))
#             if not dob_iso:
#                 raise HTTPException(status_code=400, detail="Missing/invalid DOB from sources")

#             # Gender
#             gender = _safe_gender(prefill_result.get("gender") or pan_details.get("gender")) or "Male"

#             # Email & Phone
#             email = _safe_email(prefill_result.get("email") or pan_details.get("email")) or ""
#             phone_digits = _digits_only(phone_number)
#             if not phone_digits:
#                 raise HTTPException(status_code=400, detail="Missing/invalid phone number")

#             # Address: prefer prefill list, fallback to PAN
#             best_addr = _pick_best_address(prefill_result.get("address") or [], pan_details.get("address") or {})
#             if not best_addr:
#                 raise HTTPException(status_code=400, detail="No valid address (6-digit PIN + state) to build CIBIL payload")

#             region_code = _region_code_for_state(best_addr["state"])

#             address_dict = {
#                 "StreetAddress": best_addr["street"] or "NA",
#                 "City": (best_addr["city"] or "NA").upper(),
#                 "PostalCode": int(best_addr["pin"]),          # already validated 6-digit
#                 "Region": region_code,
#                 "AddressType": 1
#             }

#             # 4) CIBIL payload (robust)
#             cibil_payload = {
#                 "CustomerInfo": {
#                     "Name": {"Forename": first_name, "Surname": last_name},
#                     "IdentificationNumber": {"IdentifierName": "TaxId", "Id": final_pan_number},
#                     "Address": address_dict,
#                     "EmailID": email,
#                     "DateOfBirth": dob_iso,                          # YYYY-MM-DD
#                     "PhoneNumber": {"Number": int(phone_digits)},
#                     "Gender": gender
#                 },
#                 "LegalCopyStatus": "Accept",
#                 "UserConsentForDataSharing": True
...
#                 cibil_data = cibil_resp.json()
#             except Exception:
#                 cibil_data = {"status": str(cibil_resp.status_code), "raw_text": await cibil_resp.text()}
#             print(f"cibil data : {cibil_data}")

#             # 6) Compute active EMI sum
#             tlp = (
#                 cibil_data.get("cibilData", {})
#                          .get("GetCustomerAssetsResponse", {})
#                          .get("GetCustomerAssetsSuccess", {})
...
#                          .get("TradeLinePartition")
#             )
#             tradelines = tlp if isinstance(tlp, list) else ([tlp] if isinstance(tlp, dict) else [])
#             active_total = 0.0
#             seen = set()
#             for item in tradelines:
#                 t = item.get("Tradeline", {}) if isinstance(item, dict) else {}
...
#             # 7) Extract borrower details (with guards)
#             try:
#                 borrower = (
#                     cibil_data.get("cibilData", {})
#                     .get("GetCustomerAssetsResponse", {})
...
#                     email_b = email_data.get("Email", "")
#                 elif isinstance(email_data, list) and email_data:
#                     email_b = email_data[0].get("Email", "")
#                 else:
#                     email_b = ""

#                 credit_score = (borrower.get("CreditScore", {}) or {}).get("riskScore")
#                 id_part = (borrower.get("IdentifierPartition", {}) or {}).get("Identifier")
#                 pan_number_b = ""
#                 if isinstance(id_part, list) and len(id_part) > 1:
#                     pan_number_b = id_part[1].get("ID", {}).get("Id", "")

#                 paddr = borrower.get("BorrowerAddress", [])
#                 pincode_b = ""
#                 if isinstance(paddr, list) and paddr:
#                     pincode_b = (paddr[0].get("CreditAddress", {}) or {}).get("PostalCode", "")

#                 user_details = {
#                     "dob": dob_formatted or dob_iso,
#                     "credit_score": credit_score,
#                     "email": email_b or email or "",
#                     "gender": borrower.get("Gender", "") or gender,
#                     "pan_number": pan_number_b or final_pan_number,
#                     "pincode": pincode_b or best_addr["pin"],
#                     "name": name_prefill or f"{first_name} {last_name}".strip(),
#                     "phone": phone_digits
#                 }
#                 print("................................................................................................ userdetail")
#                 print(user_details)
...
#                 pan_number=final_pan_number
#             )
#             return {"fallback_used": True, **fallback_result}
#         except Exception as fallback_error:
#             print("❌ Ongrid fallback also failed:", fallback_error)
#             # Propagate as 200 with detail to match your earlier behavior
#             raise HTTPException(status_code=200, detail=f"Both TransBank and Ongrid failed: {str(fallback_error)}")
...
#             # Keep the response shape consistent with your handler
#             return {
#                 "consent": "Y",
#                 "pan": fetch_data.get("pan_number"),
#                 "message": fetch_data.get("message", "OTP verified and data fetched successfully"),
#                 "phone_number": phone_number,
#                 "cibilScore": fetch_data.get("cibilScore") or (fetch_data.get("profile_detail", {}) or {}).get("credit_score"),
#                 "transId": fetch_data.get("transId") or (fetch_data.get("cibil_report", {}) or {}).get("transaction_id"),
#                 "raw": fetch_data.get("raw") or fetch_data.get("cibil_data") or fetch_data.get("cibil_report"),
#                 "approvedLenders": fetch_data.get("approvedLenders") or [],
#                 "moreLenders": fetch_data.get("moreLenders") or [],
#                 "emi_data": emi_data if emi_data is not None else {},
#                 "data": fetch_data.get("data") or fetch_data.get("cibil_data") or fetch_data.get("cibil_report"),
#                 "user_details": fetch_data.get("user_details") or fetch_data.get("profile_detail"),
#                 "source": fetch_data.get("source") or "cibil"
#             }

#         except Exception as e:
#             # You previously returned 200 with detail for failures
#             raise HTTPException(status_code=200, detail=f"{str(e)}")
